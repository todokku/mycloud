...
```
mkdir -p $HOME/.mycloud/ldap/database
mkdir -p $HOME/.mycloud/ldap/config
docker run -d \
    --name mycloud-ldap \
    --hostname ldap.mycloud.org \
    -p 389:389 \
    -p 636:636 \
    -v $HOME/.mycloud/ldap/database:/var/lib/ldap \
    -v $HOME/.mycloud/ldap/config:/etc/ldap/slapd.d \
    osixia/openldap:1.3.0
```

```
# Generate password:
docker exec mycloud-ldap slappasswd -h {SSHA} -s password

# Get example user config file
docker exec mycloud-ldap cat /container/service/slapd/assets/test/new-user.ldif

# Copy user config file over to container
docker cp ./new-user.ldif mycloud-ldap:/container/service/slapd/assets/test/mc-u-a.ldif

# Add user to LDAP
docker exec mycloud-ldap ldapadd -x -D "cn=admin,dc=example,dc=org" -w admin -f /container/service/slapd/assets/test/mc-u-a.ldif -H ldap://ldap.mycloud.org -ZZ


docker exec mycloud-ldap ldapsearch cn=billy -b "dc=example,dc=org" -s sub "(objectclass=*)"


ldapsearch -x -b dc=test,dc=com


docker exec mycloud-ldap ldapsearch -D cn=admin,dc=example,dc=org -w -b "dc=example,dc=org" "(cn=admin)"
```
...
curl -H "Content-Type: application/json" \
     -X POST \
     -d '{"strategy":"ldap", "username":"billy","password":"password"}' \
...
### Install FeathersJS LDAP plugin

### Write user "create / delete / update / authenticate" logic in feathersJS to work with OpenLDAP

### Install DEX on kubernetes, along with OpenLDAP plugin and set up RBAC
