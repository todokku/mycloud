...
class TaskServicesController {
    ...
    static async requestDeleteK8SService(topicSplit, ip, data) {
        try {
            ...
            // Delete helm service
            await this.deleteHelmService(data.service.instanceName, data.service.namespace, data.node);   
    ...
    static async deleteHelmService(serviceName, ns, node) {
        let r = await OSController.sshExec(node.ip, `helm uninstall ${serviceName}${ns ? " --namespace " + ns:""}`, true);
        if(r.code != 0) {
            console.log(JSON.stringify(r, null, 4));
